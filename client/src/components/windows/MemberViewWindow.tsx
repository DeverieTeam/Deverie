import { useEffect, useState } from "react";
import { useAuth } from "../../contexts/useAuth";
import { mainCommonsWebcontentType } from "../../types/samples/mainCommonsWebcontentType";
import BanConfirmWindow from "./BanConfirmWindow";

export default function MemberViewWindow({
  setIsSelfOpened,
  memberId,
  webcontent,
}: Props) {
  const serverAddress: string = import.meta.env.VITE_SERVER_ADDRESS;
  const { auth } = useAuth();
  
  const [data, setData] = useState<null | {
    id: number;
    name: string;
    email: string;
    is_email_displayed: boolean;
    profile_picture: string;
    role: "member" | "moderator" | "administrator";
    pronouns?: string;
    description?: string;
    inscription_date: string;
    post_count: number;
  }>(null);

  const [isBanConfirmWindowOpened, setIsBanConfirmWindowOpened] = useState<boolean>(false);

  useEffect(() => {
    if (data === null) {
      fetch(`${serverAddress}/member/${memberId}`)
        .then((response) => {
          if (!response.ok) {
            setIsSelfOpened(false);
            throw new Error("Something went wrong");
          }
          return response.json();
        })
        .then((data) => {
          setData(data);
        });
    }
  }, []);

  const closeWindow = () => {
    setIsSelfOpened(false);
  };

  const handleOpenBanConfirmWindow = () => {
    setIsBanConfirmWindowOpened(true);
  };

  return (
    <>
      <div
        className="inset-0 absolute h-[120%] w-[100%] bg-gray-400/60 z-20 -translate-y-16"
        onClick={closeWindow}
      >
        <div className="h-[100%] w-[100%] relative">
          <div className="h-screen w-screen sticky top-16">
            <div
              className="relative mx-auto px-4 py-6 xl:py-12 h-[530px] md:h-[600px] w-[290px] md:w-[600px] xl:w-[1000px] bg-neutral-50 translate-y-[20%] md:translate-y-[15%] justify-between rounded-lg shadow-sm shadow-gray-700 flex flex-col overflow-auto"
              onClick={(e) => {
                e.stopPropagation();
              }}>
              <button
                className="absolute top-4 right-4 w-7 md:w-8 h-7 md:h-8 bg-indigo-400 hover:bg-indigo-600 self-center hover:text-white text-center rounded-full shadow-sm shadow-indigo-700 hover:shadow-indigo-900"
                onClick={closeWindow}>
                <svg
                  width="800px"
                  height="800px"
                  className="m-auto w-5 md:w-6 h-5 md:h-6 bg-transparent"
                  viewBox="0 0 1024 1024"
                  xmlns="http://www.w3.org/2000/svg">
                  <path
                    fill="currentColor"
                    d="M195.2 195.2a64 64 0 0 1 90.496 0L512 421.504 738.304 195.2a64 64 0 0 1 90.496 90.496L602.496 512 828.8 738.304a64 64 0 0 1-90.496 90.496L512 602.496 285.696 828.8a64 64 0 0 1-90.496-90.496L421.504 512 195.2 285.696a64 64 0 0 1 0-90.496z"
                  />
                </svg>
              </button>
              <div className="w-full h-full flex flex-col xl:flex-row">
                <div className="xl:w-[50%] mb-6 xl:my-auto flex flex-col">
                  <img
                    className="mx-auto mb-6 h-24 md:h-[180px] xl:h-[240px] w-24 md:w-[180px] xl:w-[240px] rounded-full bg-transparent"
                    src={data?.profile_picture}
                  />
                  <p className="text-center px-4 mb-2 md:mb-4 text-indigo-500 text-2xl md:text-5xl font-bold drop-shadow">
                    {data?.name}
                  </p>
                  {data && (
                    <p className="text-center px-4 md:text-2xl">
                      {webcontent.memberWindow.role[data.role].content}
                    </p>
                  )}
                  {data && data.pronouns && (
                    <p className="text-center px-4 md:text-2xl">
                      {webcontent.memberWindow.pronounsPrefix.content}
                      {data.pronouns}
                    </p>
                  )}
                  {data?.is_email_displayed && (
                    <p className="text-center px-4 md:text-2xl break-words">{data?.email}</p>
                  )}
                  <p className="text-center px-4 md:text-2xl">
                    {webcontent.memberWindow.messageCount.content}
                    {data?.post_count}
                  </p>
                  <p className="text-center px-4 md:text-2xl">
                    {webcontent.memberWindow.inscriptionDate.content}
                    {data?.inscription_date}
                  </p>
                </div>
                <div className="xl:w-[50%] pb-6 md:pb-12 xl:pb-0 xl:my-auto gap-6 md:gap-8 xl:gap-0 justify-center flex flex-col">
                  {data && (
                    <p className="bg-neutral-100 px-2 py-4 md:mt-4 xl:mt-0 md:mx-4 xl:px-4 xl:py-6 rounded-lg shadow-sm shadow-neutral-400 text-center md:text-xl xl:text-2xl">
                      {data.description
                        ? data.description
                            .split("\n")
                            .flatMap((line: string, i: number) => [
                              line,
                              <br key={i} />,
                            ])
                        : webcontent.memberWindow.descriptionPlaceholder.content}
                    </p>
                  )}
                  <div className="justify-center flex">
                    {data &&
                      auth &&
                      (auth.role === "moderator" ||
                        auth.role === "administrator") &&
                      data.role !== "moderator" &&
                      data.role !== "administrator" && (
                        <button
                          className="py-1 px-4 md:px-8 xl:mt-12 text-center text-lg md:text-2xl hover:text-white bg-indigo-400 hover:bg-indigo-600 rounded-full shadow-sm shadow-indigo-700 hover:shadow-indigo-900"
                          onClick={handleOpenBanConfirmWindow}
                        >
                          {webcontent.memberWindow.banButton.content}
                        </button>
                      )}
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
      {isBanConfirmWindowOpened && (
        <BanConfirmWindow
          setIsSelfOpened={setIsBanConfirmWindowOpened}
          memberId={memberId}
          webcontent={{
            memberWindow: webcontent.memberWindow,
            buttons: {
              backButton: webcontent.buttons.backButton,
              confirmButton: webcontent.buttons.confirmButton
            }
          }}
        />
      )}
    </>
  );
}

type Props = {
  setIsSelfOpened: (arg0: boolean) => void;
  memberId: number;
  webcontent: mainCommonsWebcontentType;
};
