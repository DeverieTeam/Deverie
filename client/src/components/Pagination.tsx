export default function Pagination({
  ...
  const type: string = (data.replies ? 'post' : 'threads');
  const listLength: number = (type === 'post' ? data.results_length : data[0].results_length);
      ...
      <div className="text-center md:text-lg xl:text-xl">
        {type === 'post' ?
          <>
            ...
            {data && data.replies && data.replies.length === 1 ? "" : "s"}{" "}
          </>
        :
          <>
            {data && data.length}{" "}
        ...
        {webcontent.outOf.content + " " + listLength}
      </div>
      <div className="mb-12 flex">
        {data && listLength > maxPostPerPage && (
          <>
            ...
            <div className="flex-1 my-auto text-center md:text-lg xl:text-xl">
              ...
              {data && listLength
                ? Math.ceil(listLength / maxPostPerPage)
                : "????"}
            </div>
            <div className="justify-between flex">
              <div className="gap-4 md:gap-6 flex">
                <button
                  ...
                  disabled={
                    data &&
                    listLength &&
                    pagination === Math.ceil(listLength / maxPostPerPage)
                  }
                  onClick={() => {
                    setPagination(pagination + 1);
                  }}>
                ...
                <button
                  ...
                  onClick={() => {
                    if (data && listLength) {
                      setPagination(Math.ceil(listLength / maxPostPerPage));
                    }
                  }}>
                  {'>>'}
                </button>
